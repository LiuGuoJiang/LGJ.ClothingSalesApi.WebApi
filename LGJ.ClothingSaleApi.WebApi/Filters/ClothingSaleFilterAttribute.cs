...
namespace LGJ.ClothingSaleApi.WebApi.Filters
{
    /// <summary>
    /// 服装批发接口过滤器
    /// </summary>
    public class ClothingSaleFilterAttribute : TypeFilterAttribute
    {
        public ClothingSaleFilterAttribute(string categoryName) : base(typeof(ClothingSaleBusiness))
        {
            Arguments = new object[] { categoryName };
        }
        private class ClothingSaleBusiness : IAsyncActionFilter, IAsyncExceptionFilter
        {
            private IDictionary<string, object> _actionArguments;

            private readonly ILogger _logger;
            public ClothingSaleBusiness(ILoggerFactory loggerFactory, string categoryName)
            {
